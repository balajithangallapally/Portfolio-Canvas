...
export default function AdminLogin() {
  ...
  const [adminUserExists, setAdminUserExists] = useState(false);
  ...
  useEffect(() => {
    const checkAdminUser = async () => {
      try {
        const res = await apiFetch<{ adminUserExists: boolean }>("/auth/health");
        setAdminUserExists(res.adminUserExists);
      } catch (err) {
        console.error("Failed to check admin user status:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const onSubmit = async (values: LoginForm) => {
    ...
    try {
      if (isSetup) {
        // Setup mode - create the first admin user
        const res = await apiFetch<{ message: string }>("/auth/bootstrap", {
          method: "POST",
        ...
        // After setup, log them in automatically
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white">
            {!adminUserExists && !isSetup ? "Admin Setup" : isSetup ? "Create Admin Account" : "Admin Access"}
          </h1>
          <p className="text-[#6B7C99] mt-2 text-sm">
            {!adminUserExists && !isSetup
        ...
        {!adminUserExists && !isSetup ? (
          <div className="bg-[#111827] border border-blue-500/30 rounded-2xl p-8 space-y-4 shadow-[0_0_40px_rgba(0,212,255,0.05)]">
            <p className="text-[#A0B3FF] text-sm">
              No admin account found. Let's set one up!
            </p>
            <Button
          ...
          >
            ...
            <Button
              ...
              className="w-full bg-primary hover:bg-primary/90 text-[#0a0a1e] font-semibold py-5 rounded-xl shadow-[0_0_20px_rgba(0,212,255,0.25)] hover:shadow-[0_0_30px_rgba(0,212,255,0.4)] transition-all"
            >
              {isSubmitting ? (isSetup ? "Setting up..." : "Signing in...") : (isSetup ? "Create Account" : "Sign In")}
            </Button>
